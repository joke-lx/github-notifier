...
export function apply(ctx: Context, config: Config) {
  ...
  const shouldTrigger = (session: any): boolean => {
    ...
    if (config.requireAt) {
      ...
      // 方法1: 检查 elements（Koishi 标准方式）
      const elements = session.elements || []
      const hasAtInElements = elements.some(el =>
        el.type === 'at' && (el.attrs?.id === selfId || el.attrs?.qq === selfId)
      )

      // 方法2: 检查原始内容（OneBot 格式）
      const hasAtInContent = content.includes(`<at id="${selfId}"`) ||
                            content.includes(`<at qq="${selfId}"`)

      // 调试日志
      ctx.logger.debug('[qa-bot] @检测:', {
        hasAtInElements,
        hasAtInContent,
      ...
      if (!hasAtInElements && !hasAtInContent) return false
    }
